...
def find_answer_to_click(weblist_array, database_array):
    ...
    founded_questions_id = []  # массив с вопросами которые были найдены и в базе и в тесте и ответы на эти вопросы
    # были найдены и в базе и в тесте
    founded_weblist_questions = []  # массив с вопросами которые были найдены и в базе и в тесте и ответы на эти
    # вопросы были найдены и в базе и в тесте (вопросы в массиве не повторяются)
    founded_weblist_answers = []  # массив с ответами которые были найдены и в базе и в тесте
    current_answers = []  # массив с ответами на текущий в цикле вопрос. Сделан чтобы каждому элементу массива
    # "founded answers" соответствовали ответы
    ...
    last_num = 10  # костыль для того чтобы отслеживать сменился ли вопрос в следующем ниже цикле, чтобы не было
    # повторяющихся найденных вопросов в массиве "founded_questions"

    for num, each in enumerate(clear_weblist_array[0][0]):  # перебираем вложенный массив с вопросами в массиве с данным
        next_question = 0  # флаг того что ответ на вопрос дан верный и нужно перейти к следующему (если = 1)
        try:
            correct_answer = []
            answer_list_index = clear_database_array[0].index(each)  # ищем индекс вопроса во вложенном массиве с
            # вопросами в базе данных (по сути является и индексом ответа)
            ...
            founded_database_question.append(database_array[0][answer_list_index])  # добавляем в массив с вопросами
            # найденный в базе вопрос
            for answer_count in range(len(correct_answer[0])):  # перебираем все "или" в базе
                ...
                for num_answer, each_answer in enumerate(clear_weblist_array[1][num]):  # перебираем все ответы из web
                    if next_question:
                        break
                    for num_correct_answer, each_correct_answer in enumerate(correct_answer[0][answer_count]):  # переби
                        # раем все варианты правильных ответов
                        if next_question:
                            break
                        if each_answer == each_correct_answer:
                            ...
                            if last_num != num:  # если вопрос тот же самый не добавляем его по новой в массив с
                                # вопросами для лога
                                founded_weblist_questions.append(weblist_array[0][0][num])
                            current_answers.append(weblist_array[1][num][num_answer])
                            last_num = num
                            if answer_correct == len(correct_answer[0][answer_count]):  # если ответ и количество
                                # ответов совпало с базой то следующий вопрос
    ...
    if len(unidentified_question) != 0:
        return answer_link_to_click, founded_questions_id, founded_database_question, founded_database_answer, \
...
def get_answer(answer):
    ...
    for every_answer in answer:
        ...
        delimiter_index = []  # обнуляем массив с индексами, чтобы не засунуть те же самые элементы из первого елемента
        # массива "answer"
        if every_answer.find(symbol) != -1:  # если нашел в строке разделитель
            ...
            while x <= length:  # цикл для прохождения по всем символам строки
                ...
                elif last_index == index or index == -1:  # если больше нет новых символов с разделителем то выходим
                    # из цикла :
                    # x += 1
                    break
                last_index = index
            ...
            for each in range(
                ...
                if each == len(delimiter_index) - 1:  # если последний элемент массива, то добавляем все остальные
                    # символы после разделителя до конца строки
